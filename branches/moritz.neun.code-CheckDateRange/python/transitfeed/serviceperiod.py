...
class ServicePeriod(object):
  ...
  def SetDateHasService(self, date, has_service=True, problems=None):
    ...
    exception_context_tuple = (has_service and 1 or 2, 
                               problems != None and problems.GetContext() or None)
  ...
  def ValidateDate(self, date, field_name, problems, context=None):
    if date is None:
      # no exception is issued because calendar.txt might not exist or be empty
      # so there might be periods with no start_date and end_date but only 
      # service exceptions coming from calendar_dates.txt
      return False
    ...
    if re.match('^\d{8}$', date) == None:
      problems.InvalidValue(field_name,
                            'date must be an string in YYYYMMDD format',
                            date, context, problems_module.TYPE_ERROR)
      return False
    try:
      date_value = time.strptime(date, "%Y%m%d")
      if not (1900 <= date_value.tm_year <= 2100):
        problems.InvalidValue(field_name,
      ...
      problems.InvalidValue(field_name, 'could not parse date', 
                            date, context, problems_module.TYPE_ERROR)
      return False

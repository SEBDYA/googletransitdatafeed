...
function fetchSVG(embedAttrs) {
if(window.svgInstalled)
	return '<embed '+embedAttrs+'>';
else if(window.askForSVGViewer)	{
	if(navigator.appName=='Netscape') {
		return '<p>To view this page you need an SVG viewer.<a href="'+getSVGInstallPage()+'">Click here</a> for more information.</p>';
		}
	else
		return '<embed '+embedAttrs+' pluginspage="'+getSVGInstallPage()+'">';
	}
else if(window.svgViewerAvailable)
	return '<embed '+embedAttrs+' pluginspage="'+svgDownloadPage+'">';
else {
	return '<p>To view this page you need an SVG viewer. There is currently no Adobe SVG Viewer available for your browser. <a href="'+svgInfoPage+'">Click here</a> for more information.</p>'
	}
}

function emitSVG(embedAttrs) {
  document.writeln(fetchSVG(embedAttrs));
}

...
def ContextTupleToDict(context):
  ...
  if not context:
    return d
  for k, v in zip(CONTEXT_PARTS, context):
    if v != '' and v != None:  # Don't ignore int(0)
      d[k] = v
  return d
...
class ExceptionWithContext(Exception):
  def __init__(self, **kwargs):
    ...
    context and context2, if present, must be a tuple of (file_name,
    row_num, row, headers). context2 is ignored if context is present."""
    ...
    if 'context2' in kwargs:
      context = context or kwargs['context2']
      del kwargs['context2']
    if context:
      kwargs.update(ContextTupleToDict(context))

...
class TripAddStopTimeObjectTestCase(ValidationTestCase):
  def runTest(self):
    ...
    trip.AddStopTimeObject(transitfeed.StopTime(self.problems, stop2, arrival_secs=20, departure_secs=20), problems=self.problems, schedule=schedule)
    # TODO: Factor out checks or use mock problems object
    try:
      trip.AddStopTimeObject(transitfeed.StopTime(self.problems, stop1, arrival_secs=15, departure_secs=15), problems=self.problems, schedule=schedule)
      self.fail('OtherProblem exception expected')
    except transitfeed.OtherProblem:
      pass
    trip.AddStopTimeObject(transitfeed.StopTime(self.problems, stop1), problems=self.problems, schedule=schedule)
    try:
      trip.AddStopTimeObject(transitfeed.StopTime(self.problems, stop1, arrival_secs=15, departure_secs=15), problems=self.problems, schedule=schedule)
      self.fail('OtherProblem exception expected')
    except transitfeed.OtherProblem:
      pass
    trip.AddStopTimeObject(transitfeed.StopTime(self.problems, stop1, arrival_secs=30, departure_secs=30), problems=self.problems, schedule=schedule)
...
class AddStopTimeParametersTestCase(unittest.TestCase):
  def runTest(self):
    ...
    trip.AddStopTime(stop, schedule=schedule)
    trip.AddStopTime(stop, arrival_secs=300, departure_secs=360, schedule=schedule)
    trip.AddStopTime(stop, arrival_time="00:07:00", departure_time="00:07:30", schedule=schedule)
    trip.Validate(problems)
...
class GetTripTimeTestCase(unittest.TestCase):
  ...
  def testGetStopTimeTrips(self):
    ...
    rv = stop1.GetStopTimeTrips(schedule=self.schedule)

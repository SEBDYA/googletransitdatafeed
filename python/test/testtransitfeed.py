...
class LoadISO8859WarningTestCase(unittest.TestCase):
  def runTest(self):
    loader = transitfeed.Loader(
      DataPath('isowarning'),
      problems = ExceptionProblemReporterNoExpiration(),
...
class MinimalUtf8Builder(TempFileTestCaseBase):
  def runTest(self):
    ...
    schedule.AddAgency("\xc8\x8b\xc8\x8b\xc8\x8b Fly Agency", "http://iflyagency.com",
                       "America/Los_Angeles")
    service_period = schedule.GetDefaultServicePeriod()
    service_period.SetDateHasService('20070101')
    ...
    route = schedule.AddRoute(u"\u03b2\u03b2", "Beta", "Bus")
    trip = route.AddTrip(schedule, u"to remote \u020b\u020b\u020b station")
